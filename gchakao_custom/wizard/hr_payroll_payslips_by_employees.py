...
class HrPayslipEmployees(models.TransientModel):
    ...
    structure_type_id = fields.Many2one('hr.payroll.structure.type', string='Tipos de Nómina', required=True,)
    ...
    def _onchange_structure_id(self):
        for rec in self:
            ...
            if rec.structure_id:
                struct_id = self.env['hr.payroll.structure'].browse(rec.structure_id.id)
                if struct_id and struct_id.employee_ids:
                    rec.employee_ids = [(6, 0, struct_id.employee_ids.mapped('employee_id').ids)]


    @api.onchange('structure_type_id')
    def _onchange_structure_type_id(self):
        for rec in self:
            rec.structure_id = False
            rec.employee_ids = False

    department_id = fields.Many2one('hr.department')

    hr_payment_type = fields.Selection(related='structure_id.hr_payment_type')
    confidential = fields.Boolean(related='structure_id.confidential')
    structure_type_code = fields.Char(compute='_compute_structure_type_code')

    @api.depends('structure_type_id')
    def _compute_structure_type_code(self):
        for rec in self:
            rec.structure_type_code = rec.structure_type_id.code
    ...
    def compute_sheet(self):
        ...
        for contract in self._filter_contracts(contracts):
            values = dict(default_values, **{
                ...
                'anio_from': self.anio_from,
                'anio_to': self.anio_to,
            })
