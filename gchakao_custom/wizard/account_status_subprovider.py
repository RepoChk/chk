...
class AccountStatusSubproviderWizard(models.TransientModel):
    _name = 'account.status.subprovider.wizard'
    _description = 'Asistente para imprimir Estado de cuentas de un Sub-Proveedor'
    ...
    @api.depends('sub_provider_ids','company_ids', 'start_date', 'ending_date')
    ...
    def _prepare_data(self):
        ...
        for inv in self.invoice_ids:
            ...
            # Obtener el nombre del sub Sub-Proveedor usando el ID
            partner_name = inv.sub_provider_id.name if inv.sub_provider_id else 'SIN SUB-PROVEEDOR'
        ...
        return {
            ...
            'customer': ', '.join(self.sub_provider_ids.mapped('name')) if self.sub_provider_ids else 'Todos los Proveedore',
            'total': sum(inv.amount_total_usd for inv in self.invoice_ids)
        }


    def print_account_status_subprovider(self):
        data = self._prepare_data()
        if not data:
            raise UserError("No se generaron datos para el reporte.")
        return self.env.ref('gchakao_custom.action_report_account_status_subprovider').report_action(self, data={'doc': data})

    def print_account_status_subprovider_detailed(self):
        data = self._prepare_data()
        if not data:
            raise UserError("No se generaron datos para el reporte.")
        return self.env.ref('gchakao_custom.action_report_account_status_subprovider_detailed').report_action(self, data={'doc': data})